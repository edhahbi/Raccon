...
static void test_insert_and_lookup(void){
    ...
    expect_true(((dict_tv*)result.value)->type == INT, "stored value type should be INT");
...
static void test_update_existing_key(void){
    ...
    dict_set(key, (dict_tv){ .type = INT, .value.int64 = 10 });
    dict_set(key, (dict_tv){ .type = INT, .value.int64 = 99 });
...
static void test_delete_removes_key(void){
    ...
    dict_set(key, (dict_tv){ .type = INT, .value.int64 = 7 });
    dict_del(key);
